...
export default function CustomInput({className, label, icon, labelClassName, inputClassName, textArea, value, onChange, props} : InputType) {
  return (
    <div className={cn(`relative text-gray-800`, className)}>
        {
            textArea ? 
            ...
            :
            <input {...props} required value={value} onChange={onChange} type="text" name="input" id="input" placeholder="" className={cn("w-full px-4 py-3 text-xs bg-gray-100  text-gray-800 dark:text-gray-300 rounded-md peer outline-none", inputClassName)}/>
        }
        {icon}
